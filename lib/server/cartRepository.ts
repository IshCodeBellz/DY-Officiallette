...
export async function getCartLinesRaw(userId: string) {
  const cart = await prisma.cart.findUnique({ where: { userId }, include: { lines: true } });
  return cart?.lines || [];
}

export async function getCartLines(userId: string): Promise<PersistedCartLine[]> {
...
export async function replaceCart(userId: string, lines: PersistedCartLine[]) {
  ...
  if (lines.length) {
    await prisma.$transaction(lines.map(l => prisma.cartLine.create({ data: {
      cartId: cart.id,
      productId: l.productId,
      size: l.size || null,
      qty: l.qty,
