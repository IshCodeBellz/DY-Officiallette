  ...
  return (
    <section className="container mx-auto px-4">
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="text-[11px] uppercase tracking-wide text-neutral-500">Live Activity</span>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        {items.map((p: any, i: number) => (
          ...
          >
            <div className="absolute top-1 left-1 z-10 text-[11px] font-semibold bg-white/90 backdrop-blur px-1.5 py-0.5 rounded shadow">#{i+1}</div>
            <Image
              src={p.image}
              alt={p.name}
              fill
